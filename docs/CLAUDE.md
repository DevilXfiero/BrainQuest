...
- Migrations: Alembic
- Versions: Python 3.12 (managed by uv; Render pinned via `PYTHON_VERSION`), Node 22 (pinned in `apps/web/package.json`)

## Gotchas
- `NEXT_PUBLIC_*` env vars are inlined at **build time**: after changing one on Vercel, redeploy. Never put secrets in them.
- The Render free API sleeps after 15 min idle (~1 min cold start); UptimeRobot pings `/health` to keep it awake.
- CORS allows only `FRONTEND_ORIGIN` (the Vercel production URL). Preview deployments will show "API unreachable".
- `apps/web/AGENTS.md` is generated by Next.js and tells agents to read `node_modules/next/dist/docs/` — this Next version is newer than most model knowledge.
...
- Conventional commits (`feat:`, `fix:`, `docs:`, `chore:`); small commits pushed **straight to `main`** (no PRs, decided Day 1). Every push to `main` auto-deploys, so run tests/lint/build before pushing.
